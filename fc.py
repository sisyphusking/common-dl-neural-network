...
class FullConnectedLayer(object):
    def __init__(self, input_size, output_size,
        ...
        # 偏置项b,因为下一层有output_size个神经元，所以b也是个数组
    ...
    def backward(self, delta_array):
        ...
        '''
        # 式8
        self.delta = self.activator.backward(self.input) * np.dot( # 这里的delta是为下一层计算误差做的准备
            self.W.T, delta_array)   # 第一次循环时，这里的input指的是输出层的输入 ，也就是隐藏层的输出  这里的delta指的是隐藏层的误差  delta_array是输出层的误差
        self.W_grad = np.dot(delta_array, self.input.T)  # 第一次循环时，这里就是输出层梯度的值
...
class Network(object):
    ...
    def predict(self, sample):
        ...
        sample: 输入样本
        '''
        output = sample # 赋值操作 防止下次训练时破坏了原有数据集 # 每一个样本第一次运行时传入值
        for layer in self.layers:
            layer.forward(output) # 当循环到最后一层时，这里的output是上一隐藏层的输出，作为输出层的输入
            output = layer.output # 将本层的输出保存到临时变量中，作为下一层计算的输入
        return output
    ...
    def train(self, labels, data_set, rate, epoch):
        ...
        epoch: 训练轮数
        '''
        for i in range(epoch):  # i 每一轮
            for d in range(len(data_set)):  # d: 单个样本
                self.train_one_sample(labels[d],
                    data_set[d], rate)

    def train_one_sample(self, label, sample, rate):
        self.predict(sample)  # 正向传播，计算输出
        self.calc_gradient(label)  # 根据输出层输出，反向计算损失
        self.update_weight(rate)  # 更新权重

    def calc_gradient(self, label):
        delta = self.layers[-1].activator.backward(
            self.layers[-1].output
        ) * (label - self.layers[-1].output)  # 计算输出层节点误差
        for layer in self.layers[::-1]:
            layer.backward(delta)  # delta是上一层节点的误差
            delta = layer.delta
        return delta
    ...
    def loss(self, output, label):
        return 0.5 * ((label - output) * (label - output)).sum() # 损失函数
