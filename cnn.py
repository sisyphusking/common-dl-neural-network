...
# 可另外参考的实现代码：https://github.com/hsmyy/zhihuzhuanlan
...
def conv(input_array,
    ...
    '''
    channel_number = input_array.ndim  # 色阶 通道
        ...
        for j in range(output_width):
            output_array[i][j] = (
                                         get_patch(input_array, i, j, kernel_width,
                                                   kernel_height, stride) * kernel_array  # 卷积核
                                 ).sum() + bias
...
# 对numpy数组进行element wise操作 ：是指向量a和b的每个元素对应乘积，即点积
def element_wise_op(array, op):
    for i in np.nditer(array,
                       op_flags=['readwrite']):    # nditer:迭代器对象，op_flags默认只读，如果改成读写模式，就允许迭代器修改数组元素
        i[...] = op(i)  # i[...]：数组中的每一个元素
...
class Filter(object):
    def __init__(self, width, height, depth):
        ...
        self.weights_grad = np.zeros(
            self.weights.shape)  # 和weight一样大小
        self.bias_grad = 0
...
class ConvLayer(object):
    ...
    def bp_sensitivity_map(self, sensitivity_array,
        ...
        expanded_array = self.expand_sensitivity_map(
            sensitivity_array)
        # full卷积，对sensitivitiy map进行zero padding，虽然原始输入的zero padding单元也会获得残差，
        ...
        element_wise_op(derivative_array,
                        activator.backward)  # 激活函数的偏导数
        self.delta_array *= derivative_array  # 传递到上一层的误差
